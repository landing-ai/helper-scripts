...
class SegmentationProcessor:
    ...
    def process_chipping(self) -> List[np.ndarray]:
        """Process chipping masks, returns list of individual chipping instance masks"""
        chipping_masks = []
        ...
        for pred_id, pred in self.json_data['predictions']['bitmaps'].items():
            if pred['labelName'] == 'CHIPPING':
                ...
                for label in range(1, num_labels):  # Start from 1 to skip background
                    ...
                    chipping_masks.append(instance_mask)
                    
                print(f"Found {len(chipping_masks)} chipping instances")
                break
                
        return chipping_masks if chipping_masks else None
    
    def measure_distances(self, edge_top: np.ndarray, edge_bottom: np.ndarray, 
                         chipping_masks: List[np.ndarray]) -> List[Dict[str, float]]:
        """
        Measure distances between each chipping instance and edges.
        Returns list of distances for each chipping instance.
        """
        all_distances = []
        if chipping_masks is None:
            return all_distances
            
        for idx, chipping_mask in enumerate(chipping_masks):
            distances = {}
            chipping_coords = np.where(chipping_mask)
            
            # Split chipping area by central line
            above_central = chipping_coords[0] < self.central_line[0][1]
            below_central = chipping_coords[0] >= self.central_line[0][1]
            
            # Handle top edge distance
            if np.any(above_central):
                min_y_chipping = np.min(chipping_coords[0][above_central])
                edge_top_coords = np.where(edge_top)
                if len(edge_top_coords[0]) > 0:
                    edge_top_y = np.max(edge_top_coords[0])
                    if min_y_chipping < edge_top_y:
                        distances['top_distance'] = -(edge_top_y - min_y_chipping)
                    else:
                        distances['top_distance'] = edge_top_y - min_y_chipping
            
            # Handle bottom edge distance
            if np.any(below_central):
                max_y_chipping = np.max(chipping_coords[0][below_central])
                edge_bottom_coords = np.where(edge_bottom)
                if len(edge_bottom_coords[0]) > 0:
                    edge_bottom_y = np.min(edge_bottom_coords[0])
                    if max_y_chipping > edge_bottom_y:
                        distances['bottom_distance'] = max_y_chipping - edge_bottom_y
                    else:
                        distances['bottom_distance'] = -(edge_bottom_y - max_y_chipping)
    ...
    def visualize_results(self, original_image_path: str, output_path: str):
        ...
        try:
            ...
            # Draw chipping instances (different shades of green)
            chipping_masks = self.process_chipping()
            if chipping_masks is not None:
                colors = ['green', 'lime', 'forestgreen', 'seagreen', 'mediumseagreen']
                for idx, mask in enumerate(chipping_masks):
...
def main(json_path: str, image_path: str):
    try:
        ...
        # Process chipping
        chipping_masks = processor.process_chipping()
        
        # Measure distances
        distances = processor.measure_distances(edge_top, edge_bottom, chipping_masks)
